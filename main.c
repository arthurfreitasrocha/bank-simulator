...
int login() //FINALIZADO
{
    ...
    printf("\n\tFAÇA SEU LOGIN NO BANCO 99!"); printf("\n\n      Esqueceu sua senha? Digite 99 no campo NOME COMPLETO");

    printf("\n\nNOME COMPLETO: "); gets(palavra2[0]);

    if(strcmp(palavra2[0], "99") == 0)
        esquecisenha();
    ...
    if(arq == NULL)
    {
        printf("\nErro. Um ou mais dados estão inválidos ou incorretos\n\n"); system("pause");
    ...
    for(j=0; j<1;)
    {
        ...
        for(i=0; i<9; i++)
        {
            ...
            if(i == 1)
            {
                ...
                if(anosN < 18)
                {
                    printf("\n\nATENÇÃO: Você precisa ter mais de 18 anos\n\n"); system("pause");
        ...
        if(resp == 0)
        {
            char dir[100], txt[1000], comando[1000] = "enviar_email.py";
            int nconta[5], i, nconta_file=0;
            ...
            for(i = 0; i < 5; i++)
            {
                nconta[i] = rand() % 10;
                file = fopen("3.txt", "a");
                fprintf(file, "%d", nconta[i]);
                fclose(file);
            }

            file = fopen("3.txt", "r");
            fscanf(file, "%d", &nconta_file);
            fclose(file);
            ...
            //CRIANDO PASTA PARA NCONTA
            itoa(nconta_file, dir, 10);

            CreateDirectory(dir, NULL);

            strcpy(txt, dir); strcat(txt, "\\"); strcat(txt, dir); strcat(txt, "_NCONTA.txt");
            ...
            strcpy(txt, dir); strcat(txt, "\\"); strcat(txt, dir); strcat(txt, "_NOME COMPLETO.txt");
            ...
            strcpy(dir, palavra2[0]);

            //criando pasta com nome do usuário
            CreateDirectory(dir, NULL);

            //INFORMAÇÕES PESSOAIS
            for(i=0; i<9; i++)
            {
                strcpy(txt, dir); strcat(txt, "\\"); strcat(txt, dir); strcat(txt, "_"); strcat(txt, palavra1[i]); strcat(txt, ".txt");
            ...
            for(i=0; i<3; i++)
            {
                strcpy(txt, dir); strcat(txt, "\\"); strcat(txt, dir); strcat(txt, "_"); strcat(txt, strupr(contaP1[i])); strcat(txt, ".txt");
...
int esquecisenha() //FINALIZADO
{
    ...
    int resp=0, i;
    char txt[100], word[1000];

    printf("\nDIGITE SEU NOME COMPLETO: "); gets(palavra2[0]); strcpy(txt, palavra2[0]);

    strcat(txt, "\\"); strcat(txt, palavra2[0]); strcat(txt, "_");

    for(i=0; i<9; i++)
    {
        strcpy(word, txt); strcat(word, palavra1[i]); strcat(word, ".txt");
...
int transferencia() //ESTÁ COM PROBLEMAS - CRASHANDO
{
    system("cls");
...
int deposito()
{
    ...
    if(file == NULL)
    {
        ...
        else
        {
            ...
            file = fopen(txt, "a");
            fprintf(file, "\nNo dia %d/%d/%d |%s| criou um boleto no valor de R$%.2f", dia, mes, ano, strupr(palavra2[0]), valor);
            fclose(file);
